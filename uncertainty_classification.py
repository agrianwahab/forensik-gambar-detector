...
class UncertaintyClassifier:
    ...
    def _calculate_uncertainty_factors(self, results: Dict, cm_indicators: List, sp_indicators: List, au_indicators: List) -> Dict[str, float]:
        ...
        4. Modul validasi gagal (NEW: SMART FAILURE DETECTION)
        """
        factors = {}

        # NEW: Run validation before calculating uncertainty to check for failures
        try:
            from validator import ForensicValidator
            validator = ForensicValidator()
            process_results, final_score, summary_text, failed_validations = validator.validate_cross_algorithm(results)

            # Extract validation scores for each method from the failed_validations detail
            validation_scores = {}
            if hasattr(validator, 'weights'):
                # We need to run individual validations to get scores
                for technique in validator.weights.keys():
                    try:
                        if technique == 'clustering':
                            confidence, _ = validator.validate_clustering(results)
                        elif technique == 'localization':
                            confidence, _ = validator.validate_localization(results)
                        elif technique == 'ela':
                            confidence, _ = validator.validate_ela(results)
                        elif technique == 'feature_matching':
                            confidence, _ = validator.validate_feature_matching(results)
                        elif technique == 'metadata':
                            confidence, _ = validator.validate_metadata(results)
                        validation_scores[technique] = float(confidence)
                    except:
                        validation_scores[technique] = 0.0
        ...
        # NEW: Increase uncertainty if feature matching validation failed
        if validation_scores.get('feature_matching', 1.0) < 0.5:
            cm_uncertainty += 0.15  # Significant penalty for failed feature validation
        elif validation_scores.get('feature_matching', 1.0) < 0.7:
            cm_uncertainty += 0.08  # Moderate penalty for low feature validation
        ...
        factors['copy_move'] = min(cm_uncertainty, 0.35) # Increased cap to 35%

        # --- Splicing uncertainty ---
        sp_uncertainty = self.base_uncertainty + overall_uncertainty_context

        # NEW: Increase uncertainty if ELA or other supporting validations failed
        if validation_scores.get('ela', 1.0) < 0.5:
            sp_uncertainty += 0.12  # Penalty for failed ELA validation
        if validation_scores.get('localization', 1.0) < 0.5:
            sp_uncertainty += 0.10  # Penalty for failed localization validation
        ...
        factors['splicing'] = min(sp_uncertainty, 0.40) # Increased cap to 40%

        # --- Authentic uncertainty ---
        au_uncertainty = self.base_uncertainty + overall_uncertainty_context

        # NEW: Increase uncertainty if key validations failed
        failed_validations = sum(1 for score in validation_scores.values() if score < 0.5)
        if failed_validations >= 2:
            au_uncertainty += 0.20  # Major penalty if multiple validations failed
        elif failed_validations >= 1:
            au_uncertainty += 0.10  # Moderate penalty for single validation failure

        # Additional penalty if clustering validation failed (specific to authentic assessment)
        if validation_scores.get('clustering', 1.0) < 0.5:
            au_uncertainty += 0.15  # Clustering is important for authentic detection
        ...
        factors['authentic'] = min(au_uncertainty, 0.35) # Increased cap for authentic
    ...
    def _calculate_overall_uncertainty(self, uncertainty_factors: Dict[str, float]) -> float:
        ...
        # Ensure minimum uncertainty level of 5% for realistic forensic analysis
        return float(np.clip(avg_uncertainty * 0.8, 0.05, 0.35)) # Min 5%, max 35%

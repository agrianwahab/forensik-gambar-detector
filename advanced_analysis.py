...
def perform_statistical_analysis(image_pil):
    ...
    try:
        ...
        l, a_channel, b_channel = lab[:,:,0], lab[:,:,1], lab[:,:,2]
        h_channel, s, v = hsv[:,:,0], hsv[:,:,1], hsv[:,:,2]

        # Calculate statistical moments for each channel
        channels = {
            'red': r, 'green': g, 'blue': b,
            'luminance': l, 'a_channel': a_channel, 'b_channel': b_channel,
            'hue': h_channel, 'saturation': s, 'value': v
        }

        stats = {}
        for channel_name, channel_data in channels.items():
            channel_flat = channel_data.flatten()
            stats[f'{channel_name}_mean'] = float(np.mean(channel_flat))
            stats[f'{channel_name}_std'] = float(np.std(channel_flat))
            stats[f'{channel_name}_skewness'] = calculate_skewness(channel_flat)
            stats[f'{channel_name}_kurtosis'] = calculate_kurtosis(channel_flat)
            stats[f'{channel_name}_entropy'] = safe_entropy(channel_data)

        # Color correlations
        stats['rg_correlation'] = float(np.corrcoef(r.flatten(), g.flatten())[0,1]) if r.size > 1 else 0.0
        stats['rb_correlation'] = float(np.corrcoef(r.flatten(), b.flatten())[0,1]) if r.size > 1 else 0.0
        stats['gb_correlation'] = float(np.corrcoef(g.flatten(), b.flatten())[0,1]) if g.size > 1 else 0.0
        ...
        stats['r_hist_peak'] = float(np.argmax(hist_r))
        stats['g_hist_peak'] = float(np.argmax(hist_g))
        stats['b_hist_peak'] = float(np.argmax(hist_b))
        stats['r_hist_spread'] = float(np.std(np.where(hist_r > 0)[0]))
        stats['g_hist_spread'] = float(np.std(np.where(hist_g > 0)[0]))
        stats['b_hist_spread'] = float(np.std(np.where(hist_b > 0)[0]))

        # Color space inconsistencies
        lab_inconsistency = np.std(l) / (np.mean(l) + 1e-6) if np.mean(l) > 0 else 0.0
        hsv_inconsistency = np.std(v) / (np.mean(v) + 1e-6) if np.mean(v) > 0 else 0.0

        stats['lab_inconsistency'] = float(lab_inconsistency)
        stats['hsv_inconsistency'] = float(hsv_inconsistency)
        stats['overall_color_inconsistency'] = float((lab_inconsistency + hsv_inconsistency) / 2)

        # Add compatibility keys for export_utils.py
        stats['R_entropy'] = stats['red_entropy']
        stats['G_entropy'] = stats['green_entropy']
        stats['B_entropy'] = stats['blue_entropy']
        stats['overall_entropy'] = (stats['red_entropy'] + stats['green_entropy'] + stats['blue_entropy']) / 3

        return stats

    except Exception as e:
        print(f"  Warning: Statistical analysis failed: {e}")
        # Return default values with N/A indicators instead of empty dict
        return {
            'red_entropy': 'N/A',
            'green_entropy': 'N/A',
            'blue_entropy': 'N/A',
...
def analyze_compression_artifacts(image_pil):
    ...
    try:
        ...
        # Basic compression artifact analysis
        artifacts = {
            'block_artifacts': analyze_block_artifacts(gray_image),
            'dct_analysis': perform_dct_analysis(gray_image),
            'quality_estimate': estimate_compression_quality(gray_image),
            'double_compression_signs': detect_double_compression(gray_image)
        }

        # Calculate overall compression artifact score
        artifact_score = 0.0
        if 'block_variance_std' in artifacts['block_artifacts']:
            artifact_score += min(artifacts['block_artifacts']['block_variance_std'] * 10, 30)

        if 'high_freq_energy' in artifacts['dct_analysis']:
            artifact_score += min(artifacts['dct_analysis']['high_freq_energy'] * 100, 25)

        if 'estimated_quality' in artifacts['quality_estimate']:
            quality = artifacts['quality_estimate']['estimated_quality']
            if quality < 70:  # Lower quality = more artifacts
                artifact_score += (70 - quality) * 0.5

        artifacts['overall_artifact_score'] = min(artifact_score, 100.0)
...
def estimate_compression_quality(gray_image):
    ...
    try:
        # Simple quality estimation based on DCT coefficients
        dct = cv2.dct(gray_image.astype(np.float32))

        # Calculate AC energy
        ac_energy = np.sum(np.abs(dct[1:, 1:]))
        dc_energy = np.abs(dct[0, 0])

        # Quality estimation (simplified)
        quality_ratio = ac_energy / (dc_energy + 1e-6)
        estimated_quality = max(0, min(100, 100 - quality_ratio * 10))

        return {
            'estimated_quality': float(estimated_quality),
            'ac_dc_ratio': float(quality_ratio)
        }
    except Exception:
        return {'error': 'Quality estimation failed'}


def detect_double_compression(gray_image):
    """Detect signs of double JPEG compression"""
    try:
        # Simple double compression detection
        # This is a simplified version - real implementation would be more complex

        # Analyze DCT coefficient distribution
        dct = cv2.dct(gray_image.astype(np.float32))

        # Look for unusual patterns in DCT coefficients
        dct_coeffs = dct.flatten()
        hist, _ = np.histogram(dct_coeffs, bins=100, density=True)

        # Calculate histogram smoothness (double compression often creates peaks)
        hist_smoothness = np.std(hist)

        return {
            'double_compression_probability': float(min(hist_smoothness * 10, 100)),
            'histogram_smoothness': float(hist_smoothness)
        }
    except Exception:
        return {'error': 'Double compression detection failed'}

# ======================= End of Advanced Analysis Functions =======================
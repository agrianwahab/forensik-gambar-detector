...
from forensic_analysis import run_mmfusion_analysis # <-- INTEGRASI MM-FUSION
# ===========================================================================
...
try:
    ...
    from visualization import (
        ...
        create_quality_response_plot, create_color_entropy_visualization, create_advanced_combined_heatmap,
...
def display_advanced_analysis(original_pil, results):
    ...
    # Baris 1: Analisis Dasar & Deteksi Pemalsuan
    ...
    # ============== BLOK BARU UNTUK MM-FUSION ==============
    with col4:
        st.subheader("(MM-Fusion)", divider='rainbow')
        mmfusion_path = results.get('mmfusion_result_path')
        mmfusion_score = results.get('mmfusion_score')

        if mmfusion_path and os.path.exists(mmfusion_path):
            st.image(mmfusion_path, caption="Visualisasi dari model deteksi MM-Fusion.")
            
            with st.expander("Lihat Skor & Interpretasi"):
                if mmfusion_score is not None:
                    st.metric(label="Skor Deteksi Manipulasi", value=f"{mmfusion_score:.4f}")
                    if mmfusion_score > 0.7:
                        st.error("Tingkat kemungkinan manipulasi: TINGGI", icon="🚨")
                    elif mmfusion_score > 0.4:
                        st.warning("Tingkat kemungkinan manipulasi: SEDANG", icon="⚠️")
                    else:
                        st.success("Tingkat kemungkinan manipulasi: RENDAH", icon="✅")
                    st.info("Skor > 0.7 menunjukkan kemungkinan manipulasi yang tinggi.")
                else:
                    st.info("Skor MM-Fusion tidak tersedia.")
        else:
            with st.container(border=True):
                st.error("Gagal Menjalankan Analisis MM-Fusion", icon="🚨")
                st.warning("Komponen analisis MM-Fusion gagal diproses. Hasil analisis tidak lengkap.")
    # ========================================================

    st.markdown("---")
    
    # Baris 2: Analisis Iluminasi dan Deteksi Pemalsuan
    col_illum, col_ghost, col_combined = st.columns(3)
...
""")
    ...
    with col_combined:
        ...
        combined_heatmap = create_advanced_combined_heatmap(results, original_pil_array.shape[0:2])  # Pass (height, width)

        # Debug information
        heatmap_max = np.max(combined_heatmap)
        heatmap_mean = np.mean(combined_heatmap)
        st.caption(f"Debug - Heatmap max: {heatmap_max:.4f}, mean: {heatmap_mean:.4f}")

        # Overlay original image and heatmap
        fig, ax = plt.subplots(figsize=(8, 6));
        ax.imshow(original_pil_array, alpha=0.7);

        # Only show heatmap if it has meaningful values
        if heatmap_max > 0.01:
            ax.imshow(combined_heatmap, cmap='inferno', alpha=0.6);
            st.caption("Menggabungkan ELA, JPEG Ghost, dan fitur lain.")
        else:
            st.caption("Tidak ada anomali signifikan yang terdeteksi")
...
def display_statistical_analysis(original_pil, results):
    ...
    # Pass original image for color entropy analysis and calculate entropy
    results_with_image = results.copy()
    results_with_image['_original_pil'] = original_pil

    # Calculate color channel entropy values
    ...
    # Store entropy values in results for display
    results_with_image['color_entropy'] = {
        'red_entropy': r_entropy,
        'green_entropy': g_entropy,
        'blue_entropy': b_entropy,
        'mean_entropy': mean_entropy
    }

    display_single_plot(
        title="Entropi Kanal Warna",
        plot_function=create_color_entropy_visualization,
        args=[results_with_image],
        caption="Mengukur 'kerandoman' informasi pada setiap kanal warna.",
        details=f"- **Entropi Rata-rata:** `{mean_entropy:.3f}`",
        container=col3
    )
...
def display_history_tab():
    ...
    for idx, entry in enumerate(reversed(history_data)):
        ...
        # Handle both dict and string formats for analysis_summary
        analysis_summary = entry.get('analysis_summary', {})
        if isinstance(analysis_summary, dict):
            summary = analysis_summary
            result_type = analysis_summary.get('type', 'N/A')
        else:
            # If analysis_summary is a string, use it as result_type
            summary = {}
            result_type = str(analysis_summary) if analysis_summary else 'N/A'
...
def display_validation_tab_baru(analysis_results):
    ...
    with col2:
        ...
        st.dataframe(df.style.map(highlight_confidence, subset=['Kepercayaan']), use_container_width=True)
...
def main_app():
    ...
    if st.session_state.original_image:
        ...
        if st.sidebar.button("🔬 Mulai Analisis", use_container_width=True, type="primary"):
            st.session_state.analysis_results = None
            # Membuat direktori temporary lokal untuk menghindari masalah path sistem
            ...
            with st.spinner('Melakukan analisis 17 tahap... Ini mungkin memakan waktu beberapa saat.'):
                try:
                    ...
                    # ================== PANGGILAN INTEGRASI MM-FUSION ==================
                    if results:
                        with st.spinner("Menjalankan analisis tambahan MM-Fusion..."):
                            mmfusion_path, mmfusion_score, mmfusion_heatmap = run_mmfusion_analysis(temp_filepath)
                            if mmfusion_path:
                                results['mmfusion_result_path'] = mmfusion_path
                                results['mmfusion_score'] = mmfusion_score
                                results['mmfusion_heatmap'] = mmfusion_heatmap # <-- SIMPAN HEATMAP
                                st.success("Analisis MM-Fusion selesai.")
                            else:
                                st.error("Analisis MM-Fusion gagal dan tidak menghasilkan output.", icon="🚨")
                    # =================================================================

                    st.session_state.analysis_results = results

                    # Simpan hasil analisis ke riwayat secara otomatis
                    if results and 'classification' in results:
                        try:
                            # Buat thumbnail untuk riwayat
                            thumbnail_dir = "thumbnails"
                            os.makedirs(thumbnail_dir, exist_ok=True)

                            # Simpan thumbnail dari gambar asli
                            thumbnail_filename = f"thumb_{int(time.time())}_{st.session_state.original_image_name}"
                            thumbnail_path = os.path.join(thumbnail_dir, thumbnail_filename)

                            # Buat thumbnail (ukuran lebih kecil)
                            ...
                            save_analysis_to_history(
                                ...
                                results.get('classification', {}).get('type', 'Unknown'),
                                results.get('processing_time', 0),
                                thumbnail_path
                            )

                        except Exception as history_error:
                            print(f"Warning: Gagal menyimpan ke riwayat: {history_error}")
                    ...
                    # Hapus file temp setelah analisis selesai

...
def _enhance_ela_signal(ela_array, original_array, scale_factor):
    """Apply advanced signal enhancement to ELA with improved overflow protection"""
    ...
    # Improved contrast factor calculation with caps to prevent extreme values
    contrast_factor = 1.0 + min(contrast_std / 100.0, 2.0)  # Cap at 3.0x maximum

    # Apply adaptive scaling with overflow protection
    adaptive_scale = scale_factor * contrast_factor
    scaled_ela = ela_array * adaptive_scale

    # Better clamping to prevent overflow - handle dtype properly
    scaled_ela = np.clip(scaled_ela, 0, 255)
    ...
    # Local contrast enhancement using CLAHE with conservative settings
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
    contrast_enhanced = clahe.apply(enhanced_ela.astype(np.uint8)).astype(float)

    # Final clamping to ensure no overflow
